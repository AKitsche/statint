...
As an alternative analysis of the spacing by variety interaction the spacing factor could also be suggested as quantitative. In this case the comparison among regression slopes between varieties can be set up. Again it is assumed that interest is in the comparisons of the regression slopes berween the different growth types and in comparisons of the varieties within the different growth types. 

For tha sake of simpicity we fit a linear mixed model assuming the Block as random factor. 
...
fm1 <- lme(Yield ~ Variety+Variety:Spacing-1, data=beans, 
                                              random= ~ 1|Block)
#Generating user defined contrasts of regression slopes
